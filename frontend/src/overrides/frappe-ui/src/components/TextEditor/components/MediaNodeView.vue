<!--
  Override of frappe-ui's TextEditor MediaNodeView.vue (image/video node view)
  via the osOverrideTheme Vite plugin. ONLY change from upstream: the
  "Add caption" placeholder is wrapped in __() so it is translatable. Every
  frappe-ui-internal import is repointed at the original file in node_modules so
  it resolves outside src/overrides — in particular `localFileMap` MUST stay the
  same module instance the image-extension uses, or uploads break. Keep in sync
...
import { localFileMap } from '../../../../../../../node_modules/frappe-ui/src/components/TextEditor/extensions/image/image-extension'
import { ErrorMessage } from '../../../../../../../node_modules/frappe-ui/src/components/ErrorMessage'
  ...
  <NodeViewWrapper as="div" :class="node.attrs.float ? wrapperClasses(node.attrs.float) : 'my-2'">
    <div ref="containerRef" class="group relative overflow-hidden not-prose rounded-[2px]" :class="[
      ...
      <input v-if="(node.attrs.alt || showCaption) && !node.attrs.error" v-model="caption"
        ...
        :placeholder="__('Add caption')" :disabled="!isEditable" @change="updateCaption" @keydown="handleKeydown" />
