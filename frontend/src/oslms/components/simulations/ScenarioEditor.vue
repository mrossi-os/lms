	...
	<form @submit.prevent="onSave" class="space-y-6">
		...
		<div class="flex items-center justify-between">
			<h2 class="text-lg font-semibold text-ink-gray-9">
				{{ scenarioName ? __('Modifica scenario') : __('Nuovo scenario') }}
			</h2>
		...
		<div class="grid grid-cols-2 gap-4">
			<FormControl
				...
				:description="__('Età, ruolo, contesto, stato emotivo iniziale.')"
				required
			/>
			<FormControl
				...
				:description="__('Variabili randomizzate vengono sostituite al runtime.')"
				required
			/>
		</div>
		...
		<!-- Rubric link -->
		<Autocomplete
			v-model="model.evaluation_rubric"
			:options="rubricOptions"
			:label="__('Rubrica di valutazione')"
			required
		/>
		...
		<div>
			<div class="flex items-center justify-between mb-2">
				<div class="text-sm font-medium text-ink-gray-9">{{ __('Obiettivi formativi') }}</div>
				<Button size="sm" variant="ghost" @click="addObjective">+ {{ __('Aggiungi') }}</Button>
			</div>
			<div class="space-y-2">
				<div
				...
				>
					...
					<input
						...
						:placeholder="__('Peso')"
					/>
					<Button variant="ghost" size="sm" @click="removeObjective(i)">×</Button>
				</div>
			</div>
		</div>
		...
		<div>
			<div class="flex items-center justify-between mb-2">
				<div class="text-sm font-medium text-ink-gray-9">{{ __('Variabili scenario') }}</div>
				<Button size="sm" variant="ghost" @click="addVariation">+ {{ __('Aggiungi') }}</Button>
			</div>
			<div class="space-y-2">
				<div
				...
				>
					<input
						...
						type="text"
						class="w-36 rounded-md border border-outline-gray-2 px-2 py-1 text-sm"
					...
					></textarea>
					<Button variant="ghost" size="sm" @click="removeVariation(i)">×</Button>
				</div>
			</div>
		</div>
		...
		<div class="grid grid-cols-2 gap-4">
			<FormControl
				v-model="model.provider_override"
				type="select"
				:label="__('Provider LLM (override)')"
				:options="['auto', 'openai', 'gemini', 'deepseek', 'anthropic', 'mock']"
			/>
			<FormControl
				v-model="model.model_override"
				type="text"
				:label="__('Modello (override, opzionale)')"
				:placeholder="__('Vuoto = default provider')"
			/>
		</div>
	</form>
</template>
...
import { Autocomplete, Button, FormControl, createResource, toast } from 'frappe-ui'
...
const rubricRes = createResource({
	url: 'os_lms.os_lms.ai.simulations.api.list_my_rubrics',
	auto: true,
})
const rubricOptions = computed(() =>
	(rubricRes.data || []).map((r) => ({ label: r.rubric_name, value: r.name })),
)
