...
from lms.lms.utils import PRIVILEGED_ROLES as LMS_PRIVILEGED_ROLES
from lms.lms.utils import get_lms_route

# OSLMS-CUSTOM: Valutatore may save (grade) submissions of its own batch students
# Roles allowed to save a submission that belongs to somebody else and to set the
# grading fields (status, comments, evaluator) — i.e. the ones that grade.
# "Valutatore" is the os_lms custom role: it is scoped to its own batches, but that
# scoping is already applied by the time this runs, since Document.save checks the
# write permission — and with it os_lms.os_lms.valutatore.submission_has_permission —
# before validate. It is added to a module-local set on purpose: the shared
# lms.lms.utils.PRIVILEGED_ROLES is also used by LMS Certificate Request and
# LMS Course Progress, where the Valutatore must NOT gain any power.
PRIVILEGED_ROLES = LMS_PRIVILEGED_ROLES | {"Valutatore"}
