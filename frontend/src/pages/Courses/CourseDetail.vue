...
const tabs = computed<DetailTab[]>(() => [
	{
		...
		icon: 'lucide-list',
		// OSLMS-CUSTOM: a course valutatore gets the tabbed view too (read-only
		// Overview only); every other tab stays admin-only.
		when: showTabs.value,
		flow: true,
	},
	{
	...
	// globally. Students no longer get a Simulations course tab: they launch
	// simulations and review past attempts through the floating launcher button.
	{
		key: 'simulations',
...
watch(course, () => {
	...
	// OSLMS-CUSTOM: so may a member of a program holding the course (in_member_program).
	if (
		!isAdmin.value &&
