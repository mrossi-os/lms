<template>
	...
	>
		<template #filters>
			<!-- OSLMS-CUSTOM: no LMS Quiz filter for the per-batch Valutatore, who cannot list quizzes -->
			<Link
				v-if="!isValutatoreOnly"
				doctype="LMS Quiz"
		...
		<!-- OSLMS-CUSTOM: the Valutatore reads submissions, never deletes them -->
		<template
			v-if="!isValutatoreOnly"
			#selection-actions="{ unselectAll, selections }"
		>
			<Button
				variant="ghost"
				:label="__('Delete')"
...
const valutatoreOnlyBack = computed(() =>
	isValutatoreOnly.value ? { name: 'Batches' } : null
)
...
const listOptions: ListViewOptions = {
	// OSLMS-CUSTOM: no row selection (hence no bulk delete) for the Valutatore
	selectable: !isValutatoreOnly.value,
	showTooltip: false,
