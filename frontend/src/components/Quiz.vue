			...
			<div v-for="(question, qtidx) in questions" :key="question.name">
				...
				>
					...
					<div class="flex items-center mt-8 gap-4">
						...
						hand the quiz in from any question. Submit never shows in an
						author preview, where nothing may be submitted. -->
			...
			<div class="border rounded-lg overflow-hidden">
				...
				<div class="p-10 space-y-2 text-center">
					...
					<div class="flex items-center justify-center gap-x-2 pt-1">
						<!-- OSLMS-CUSTOM: Try Again is the primary action after a submission -->
						<Button
							variant="solid"
							@click="resetQuiz()"
							v-if="
...
const setupTimer = () => {
	// OSLMS-CUSTOM: never clobber a countdown that is already running: a reload
	// can fire this again while an attempt is in progress.
...
const checkAnswer = () => {
	...
	createResource({
		...
		onSuccess(data) {
			...
			// OSLMS-CUSTOM: a checked answer marks the question attempted
			recordCurrentAttempt()
			if (!quiz.data.show_answers) {
				resetQuestion()
			}
...
const resetQuiz = () => {
	// OSLMS-CUSTOM: a new attempt clears the submit-once flag and any countdown
	submitting = false
	stopTimer()
