<!--
  Override of frappe-ui's TextEditor InsertIframe.vue (the Embed dialog) via the
  osOverrideTheme Vite plugin. Changes from upstream:
    1. All user-facing dialog strings are wrapped in __() (template) / window.__
       (script) so the hardcoded English becomes translatable.
    2. Imports repointed at the originals (barrel -> 'frappe-ui'; ./utils ->
       node_modules) so they resolve outside src/overrides.
  Keep in sync with node_modules/.../extensions/iframe/InsertIframe.vue after any
...
<template>
  <div>
    ...
    <Dialog
      v-model="showDialog"
      :options="{ title: __('Insert Embed'), size: 'md' }"
    >
      <template #body-content>
        <div class="space-y-4">
          <div>
            <label class="block text-sm-medium text-ink-gray-7 mb-2">
              {{ __('URL or Embed Code') }}
            </label>
            <Textarea
              ref="urlInput"
              v-model="embedUrl"
            ...
            >
              ✓ {{ __('Valid {0} URL').format(platformInfo.platform) }}
            </p>
          </div>
        </div>
      </template>
...
import { Dialog, Button, TextInput, Textarea } from 'frappe-ui'
...
import {
  ...
  ALLOWED_DOMAINS
} from '../../../../../../../../node_modules/frappe-ui/src/components/TextEditor/extensions/iframe/utils'
...
function validateUrl() {
  ...
  if (!isValidUrl.value) {
    urlError.value = window.__('Please enter a supported URL or iframe embed code')
  }
}
...
function insertIframe() {
  ...
  if (success) {
    ...
    urlError.value = window.__('Failed to insert embed. Please check the URL and try again.')
  }
}
