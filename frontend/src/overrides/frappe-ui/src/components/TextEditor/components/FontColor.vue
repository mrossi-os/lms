<!--
  Override of frappe-ui's TextEditor FontColor.vue (the toolbar "Font Color"
  popover) via the osOverrideTheme Vite plugin. Changes from upstream:
    1. Section headers + colour tooltips/aria-labels translated (upstream
       hardcodes English).
...
<template>
  <Popover transition="default">
    ...
    <template #body-main>
      <div class="p-2">
        <div class="text-sm text-ink-gray-7">{{ __('Colore testo') }}</div>
        ...
        <div class="mt-2 text-sm text-ink-gray-7">{{ __('Colore sfondo') }}</div>
        <div class="mt-1 grid grid-cols-6 gap-1">
          <Tooltip
            class="flex"
