...
class SessionOrchestrator:
    ...
    def start_session(
        ...
        *,
        ...
        """Create a Session, generate the variant, persist the first customer turn.
        ...
        first_turn = self._persist_turn(
            session=session,
            role="assistant",
            text=_first_customer_line(variant),
    ...
    def send_message(self, *, session_id: str, user_text: str) -> "frappe._dict":
        """Append a user turn, ask the customer to reply, persist the assistant turn."""
        ...
        try:
            ...
            if attack:
                ...
                t0 = time.monotonic()
                response = self._ask_customer(session, persona)
...
def validate_quota(doc, method=None) -> None:
    ...
    if count >= quota:
        raise QuotaExceededError(
            _("Daily simulation quota of {0} reached for {1}.").format(quota, student)
        )
...
def _first_customer_line(variant: ScenarioVariant) -> str:
