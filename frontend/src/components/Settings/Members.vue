		...
		<div v-if="displayedMembers.length">
			<List class="list-row-px-3">
				<ListRows
					...
					v-slot="{ item: member }"
				>
					<!-- OSLMS-CUSTOM: clicking a member row opens the edit-member modal; avatar and name open the profile -->
					<ListRow class="py-2.5" @click="openEditMember(member)">
						<ListCell @click.stop="openProfile(member.username)">
							<Avatar
