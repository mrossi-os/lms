...
import { MODAL_FORM_ROUTES } from './paletteTypes'
import {
	categoryById,
	visibleCategories,
	visibleNavTargets,
...
// The store's userResource is the same resource main.js provides as $user.
...
const searchableSections = computed<PaletteItem[]>(() => {
	...
	return [
		...paletteCategories(links).map((entry) => ({
...
// The job board is not part of the product, so its category is never offered,
// whatever the sidebar flags say.
const paletteCategories = (links: any) =>
	visibleCategories(links, sidebarVisibility.value).filter(
		(entry) => entry.id !== 'jobs'
	)

// OSLMS-CUSTOM: "Search for" entry to the /search page, kept after upstream dropped it
// Without it the full search page cannot be reached from the palette.
const searchPageItem = computed<PaletteItem | null>(() => {
	const term = query.value.trim()
	if (!term || scope.value) return null
	return {
		title: __('Search for ') + `"${term}"`,
...
const groups = computed<PaletteGroup[]>(() => {
	...
	// OSLMS-CUSTOM: the "Search for" row is the last row while searching, after every hit
	const searchPage = searchPageItem.value
		? [{ title: 'Search', items: [searchPageItem.value], isStale: stale }]
		: []
	const source = searched
		? [
				...(sections.length ? [{ title: __('Jump to'), items: sections }] : []),
				...hits,
				...searchPage,
			]
		: browseGroups.value
...
const navigateTo = (route: PaletteRoute | null) => {
...
/* The default <mark> renders black text, unreadable in dark mode. */
mark {
