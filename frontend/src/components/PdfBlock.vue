<template>
	<!-- OSLMS-CUSTOM: upstream pdf.js viewer adopted at v2.60.0 (owner decision); ours adds
	     private-URL handling (fileURL) and translated labels -->
...
// OSLMS-CUSTOM: labels go through window.__ (set by translationPlugin); outside the
...
// The encoded URL still goes through upstream's scheme allowlist before it is bound.
const safeFileURL = computed(() => safeUrl(fileURL.value))
