<!--
  Override of frappe-ui's Autocomplete
  (node_modules/frappe-ui/src/components/Autocomplete/Autocomplete.vue) via the
  osOverrideTheme Vite plugin. The ONLY functional change from upstream is the
  dropdown search box placeholder: hardcoded "Search" -> `__('Search')` so it
  localises (used e.g. by the evaluation-schema picker).

  Import rewrites are mandatory: the plugin does NOT re-route imports made BY an
  override file, so bare relative frappe-ui-internal imports would resolve inside
  src/overrides/ and 404. Barrel-exported deps come from 'frappe-ui'; the rest
  point straight at node_modules (6 × ../ = depth of this dir to frontend/).
  Being an app-wide input, keep this file byte-faithful with upstream after any
  frappe-ui bump — re-diff and re-sync every time.
-->
<template>
  <Combobox
...
import { warnDeprecated } from '../../../../../../node_modules/frappe-ui/src/utils/warnDeprecated'
import { autocompleteDeprecationSuppressed } from '../../../../../../node_modules/frappe-ui/src/components/Autocomplete/deprecationKey'
import type {
  AutocompleteOptionGroup,
  AutocompleteOption,
  AutocompleteProps,
  Option,
} from '../../../../../../node_modules/frappe-ui/src/components/Autocomplete/types'
