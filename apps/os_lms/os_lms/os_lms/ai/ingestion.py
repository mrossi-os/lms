...
def normalize_lesson_text(lesson):
    ...
    parser = LessonContentParser(lesson_doc)
...
def get_redis_client():
    ...
    """
    print(f"Configurazione redis { frappe.conf.get("redis_vector_store")}")
    redis_url = (
        frappe.conf.get("redis_vector_store")
        or frappe.conf.get("redis_queue")
        or "redis://localhost:6379"
    )
    return redis.from_url(redis_url, db=2)
...
def ensure_vector_index(client):
    ...
    try:
        ...
        definition = IndexDefinition(
            prefix=[f"{prefix}:chunk:"], index_type=IndexType.HASH
        )
...
def ask_chat(course_id, lesson_id, question):
    ...
    rag = RagDB()
    context_chunks = rag.search(course_id, lesson_id, question)

    context_chunks_ = vector_search(question, course_id, lesson_id, settings)
...
def ingest_lesson(lesson_id):
    ...
    rag = RagDB()
    rag.ingest_data(lesson.course, lesson.name, text)
