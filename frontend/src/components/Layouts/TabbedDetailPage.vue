			...
			>
				<template #tab-item="{ tab }">
					...
					>
						...
						{{ tabLabel(own(tab)) }}
						<!-- OSLMS-CUSTOM: unread notification badge per tab (batch Classes/Announcements/Discussions) -->
						<Badge v-if="own(tab).badge" theme="red" size="sm">{{ own(tab).badge }}</Badge>
					</button>
				</template>
...
export interface DetailTab {
	...
	// OSLMS-CUSTOM: optional unread count rendered next to the tab label
	badge?: number
}
</script>
...
import { Badge, Tabs } from 'frappe-ui'
