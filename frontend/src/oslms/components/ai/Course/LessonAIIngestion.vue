<template>
	...
	<div class="grid grid-cols-2 gap-5 w-5/6 mx-auto p-4">
		...
		<div>
			<Button :variant="needsUpdate || status !== 'completed' ? 'solid' : 'outline'" size="sm" class="w-full"
				:loading="isIngesting" :disabled="!lessonId || status === 'pending'" @click="startIngestion">
				<template v-if="status === 'not_ingested'">
					{{ __('Index for AI') }}
				</template>
				<template v-else-if="status === 'pending'">
