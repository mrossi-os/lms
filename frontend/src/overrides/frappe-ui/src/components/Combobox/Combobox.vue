<!--
  OVERRIDE of frappe-ui's Combobox (node_modules/frappe-ui/src/components/Combobox/Combobox.vue).

  Reason: the empty-state text is hardcoded in English with no slot/prop to
  customize it, so it could not be translated from the outside. This file is a
  VERBATIM copy of the upstream component with a SINGLE divergence:

    - the <ComboboxEmpty> text (search "No results found" below) is wrapped in
      __() so it goes through the LMS translation layer.

  When upgrading frappe-ui, re-copy the upstream file and re-apply only that one
  line. Keep everything else identical to minimize drift.
-->
...
// Type-only import resolved by @vue/compiler-sfc for the defineProps macro. The
// SFC compiler resolves this on disk and does NOT honor Vite aliases, and there
// is no types.ts next to this override, so we point straight at the upstream
// file in node_modules (relative path, not the package subpath which frappe-ui's
// `exports` field blocks). Being `import type`, it is erased after macro
// compilation and never reaches the bundle.
import type {
  CustomOption,
  SimpleOption,
  GroupedOption,
      ...
      <ComboboxPortal>
        ...
        >
          ...
          >
            ...
            >
              <!-- OVERRIDE: wrapped in __() for translation (only divergence from upstream) -->
              {{ searchTerm ? __('No results found for "{0}"').format(searchTerm) : __('No results found') }}
            </ComboboxEmpty>
            <template
