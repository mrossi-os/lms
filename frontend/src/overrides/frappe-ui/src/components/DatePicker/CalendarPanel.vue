<!--
  Full-copy override of frappe-ui's DatePicker/CalendarPanel.vue.

  frappe-ui renders the calendar chrome with hardcoded English:
    - weekday initials  ['S','M','T','W','T','F','S']
    - month labels      imported from ./utils as ['Jan',…,'Dec']
    - the "Today"/"Now" action button label (passed in as `todayLabel`)
  None of these go through `__()`, so they stay English regardless of the
  loaded locale. This override translates the *visible* calendar to Italian.
  It is shared by every picker (DatePicker / DateTimePicker / DateRangePicker),
  so translating it here localises the calendar app-wide.

  Only diffs vs the original:
    1. WEEKDAYS → Italian initials (kept Sunday-first: the grid is generated
       Sunday-first by ./utils generateWeeks, so the labels must match).
    2. `months` defined locally in Italian instead of imported from ./utils
       (a `.ts` — osOverrideTheme only intercepts `.vue`, and it holds the
       hardcoded English list anyway).
    3. Today/Now button label routed through `__()` ("Today"→"Oggi",
       "Now"→"Adesso").
    4. cell aria-label "(Today)" → "(Oggi)".

  Import rewrites are mandatory — osOverrideTheme does NOT re-route imports made
  BY an override file, so bare relative imports resolve inside src/overrides/
  and 404. Barrel-exported deps come from 'frappe-ui'; types point straight at
  node_modules (6 × ../ = depth of this dir to frontend/).
-->
<template>
...
import type { DatePickerViewMode as ViewMode } from '../../../../../../node_modules/frappe-ui/src/components/DatePicker/types'

// Italian month labels — replaces the hardcoded English list imported from
// ./utils (a `.ts` the override plugin can't intercept). Same 3-letter,
// abbreviated style as upstream (shown both in the header and the month grid).
const months: string[] = [
  'Gen',
  'Feb',
  'Mar',
...
// Italian weekday initials, kept Sunday-first to match the grid generated by
// ./utils generateWeeks (dayjs default locale = Sunday-first): D L M M G V S.
