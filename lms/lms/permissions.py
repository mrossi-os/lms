...
def resolve_lesson_access(lesson: str, *, user: str | None = None) -> tuple[bool, bool]:
	...
	- ``can_access``: ``is_instructor`` OR valutatore of a batch holding the course OR
	  enrolled member OR (published course AND include_in_preview AND guest access allowed).
	...
	try:
		...
		if can_modify_course(lesson_row.course):
			return True, True
		# OSLMS-CUSTOM: Valutatore branch, deliberately after the can_modify_course check
		...
		# without progress tracking). Returning is_instructor=False keeps instructor-only
		# media out of reach (can_access_lesson(instructor_only=True) returns is_instructor).
	...
	try:
		...
		for batch in assessment_batches:
			if batch and (
				...
				# OSLMS-CUSTOM: the batch's own Valutatore reviews its quiz assessments.
				or is_batch_valutatore(batch, user)
			):
				return True
